...
- Dependency vulnerabilities: triaged 2026-08-11, now down to **1 remaining** (`npm audit` was 26). Fixed: `next-auth`/`@auth/core`/`@auth/prisma-adapter` critical Auth.js advisories (`957cd27`), `next` 16.2.4→16.2.11 (`957cd27`), and the rest of the transitive dev/build deps via `npm audit fix` — `postcss`, `svgo`, `js-yaml`, `brace-expansion`, `axios`, `form-data`, `prisma` 7.8.0→7.9.1, `hono`/`@hono/node-server` via `@prisma/dev` (`b0aac2f`, lockfile-only, no `package.json` edits needed since existing semver ranges already allowed them). **Still open**: `swiper` (critical, prototype pollution) — fix requires a forced breaking major bump (11.x→14.1.0) and the package isn't imported anywhere in app code (only its CSS is referenced in `globals.css`), so the standing recommendation is to remove the dependency rather than force-upgrade it. Not yet done.
